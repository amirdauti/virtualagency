...
export function ChatPanel({ agentId }: ChatPanelProps) {
  ...
  const [automationIntervalMinutes, setAutomationIntervalMinutes] =
    useState<number>(60);
  ...
  const handleCreateAutomation = useCallback(() => {
    ...
    const automation: AgentAutomation = {
      ...
      nextRunAt: new Date(
        now + automationIntervalMinutes * 60_000
      ).toISOString(),
    };
  ...
  const handleRunAutomationNow = useCallback(
    async (automation: AgentAutomation) => {
  ...
  const handlePaste = useCallback(
    async (e: ClipboardEvent<HTMLTextAreaElement>) => {
          ...
          for (let i = 0; i < items.length; i++) {
            ...
            if (isTauri()) {
              ...
              try {
                ...
                console.log(
                  "[ChatPanel] Tauri file APIs not available, falling back to browser mode:",
                  tauriErr
                );
                const objectUrl = URL.createObjectURL(blob);
                console.log(
                  "[ChatPanel] Created object URL for pasted image:",
                  objectUrl
                );
      ...
      if (!isTauri()) {
        console.log(
          "[ChatPanel] Not in Tauri mode, skipping native clipboard API"
        );
        return;
      }
      ...
      try {
        ...
        if (imageData) {
          ...
          if (rgbaData && rgbaData.length > 0) {
            console.log(
              "[ChatPanel] Got image from native clipboard:",
              width,
              "x",
              height
            );
            ...
            if (ctx) {
              ...
              if (blob) {
                ...
                console.log(
                  "[ChatPanel] File written via native API:",
                  filePath
                );
        ...
        console.log(
          "[ChatPanel] Native clipboard API failed or no image:",
          err
        );
  ...
  const handleFileInputChange = useCallback(
    (e: ChangeEvent<HTMLInputElement>) => {
      ...
      console.log(
        "[ChatPanel] Browser file input selected:",
        files.length,
        "files"
      );
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        console.log(
          "[ChatPanel] Created object URL for selected image:",
          objectUrl
        );
    ...
    <>
      ...
      {promptKind === "scheduled" && (
        ...
        >
          <div
            style={{
              ...automationComposerControlsStyle,
              gap: isMobile ? 10 : 8,
            }}
          >
            ...
            <select
              ...
              onChange={(e) =>
                setAutomationIntervalMinutes(Number(e.target.value))
              }
              disabled={sending}
              style={{
                ...selectStyle,
            ...
            <button
              ...
              disabled={
                sending || (!input.trim() && !scheduledTaskDescription.trim())
              }
              style={{
            ...
            <div key={automation.id} style={automationCardStyle}>
              <div style={automationCardTopStyle}>
                <div style={automationTitleStyle}>
                  {automation.taskDescription || automation.prompt}
                </div>
              ...
              <div style={automationMetaStyle}>
                ...
                {automation.lastRunAt
                  ? ` • Last: ${new Date(automation.lastRunAt).toLocaleString()}`
                  : ""}
              </div>
              <div style={automationActionsStyle}>
                <label style={automationToggleLabelStyle}>
                  <input
                    ...
                    onChange={(e) =>
                      handleToggleAutomation(automation.id, e.target.checked)
                    }
                    style={checkboxStyle}
                  />
                  Enabled
    ...
    >
      <div
        style={{
          ...
          height: "100%",
          gap: isMobile ? 6 : 12,
          padding: isMobile ? "8px 10px 10px 10px" : "16px 16px 20px 16px",
          width: "100%",
        }}
      >
        {isMobile && (
          ...
          >
            {attachedImages.map((img, index) => {
              ...
              const fileName = img.file
                ? img.file.name
                : img.path.split("/").pop() || "image";
              console.log(
                "[ChatPanel] Image preview src:",
                img.path,
                "->",
                imgSrc
              );
              return (
                <div key={`${img.path}-${index}`} style={imagePreviewStyle}>
                  ...
                  <span style={imageNameStyle}>{fileName.slice(0, 15)}...</span>
                </div>
              );
            })}
          </div>
          ...
          {isAgentWorking ? (
            ...
            <button
              ...
              style={{
                ...
                border:
                  "1px solid " + (!canSend || sending ? "#3c3c3c" : "#0098ff"),
                borderRadius: 6,
